...
class CurrentReadingBookShelf extends Component {
  render = () => (
  ...
  _renderBooks = () => {
    ...
    return _.map(bookshelf, (books, index) => {
      return <div key={index}>{books.map((book, key) => <CurrentReadingBookCard key={key} { ...book }/>)}</div>
    })
  }
}

export default CurrentReadingBookShelf;
